...
class PympesaTests(unittest.TestCase):
    ...
    def setUp(self):
        if ENV == "production":
            access_token = pympesa.oauth_generate_token(
                os.getenv("PROD_MPESA_CONSUMER_KEY"),
                os.getenv("PROD_MPESA_CONSUMER_SECRET")).json().get("access_token")
            print "token: " + access_token
            self.client = pympesa.Pympesa(access_token)
            self.short_code_1 = os.getenv("PROD_MPESA_SHORT_CODE_1")
            self.initiator_name_sc_1 = os.getenv("PROD_MPESA_INITIATOR_NAME_SC_1")
            self.security_credential_sc_1 = \
                os.getenv("PROD_MPESA_SECURITY_CREDENTIAL_SC_1")
            self.short_code_2 = os.getenv("PROD_MPESA_SHORT_CODE_2")
            self.test_msisdn = os.getenv("PROD_MPESA_MSISDN")
            self.online_short_code = os.getenv("PROD_MPESA_ONLINE_SHORT_CODE")
            self.online_pass_key = os.getenv("PROD_MPESA_ONLINE_PASS_KEY")
            self.initiator_security_credential = \
                os.getenv("PROD_MPESA_INITIATOR_SECURITY_CREDENTIAL")
            self.validation_url = os.getenv("PROD_MPESA_VALIDATION_URL")
            self.confirmation_url = os.getenv("PROD_MPESA_CONFIRMATION_URL")
        else:
            access_token = pympesa.oauth_generate_token(
    ...
    def test_c2b_simulate_transaction(self):
        resp = self.client.c2b_simulate_transaction(
            ...
            ShortCode=int(self.short_code_1)
        )
        print resp.json()
        self.assertEqual(resp.status_code, 200)
    ...
    def test_lipa_na_mpesa_online_query(self):
        timenow = datetime.datetime.strftime(datetime.datetime.now(), "%Y%m%d%H%M%S")
        resp = self.client.lipa_na_mpesa_online_query(
                BusinessShortCode=self.online_short_code,
                Password=base64.b64encode(
                    self.online_short_code +\
                    self.online_pass_key +\
